...
type Obligation = { name: string; cadence: string; deadline: string; href: string; statuses: Status[] };

const obligations: Obligation[] = [
  { name: "IVA", cadence: "Mensual", deadline: "15 días hábiles · período siguiente", href: "/declaraciones/iva", statuses: ["pagada", "pagada", "pagada", "pagada", "pagada", "pendiente", "futura", "futura", "futura", "futura", "futura", "futura"] },
  { name: "IVSS · Seguro Social", cadence: "Mensual", deadline: "5 días continuos · período siguiente", href: "/declaraciones/ivss", statuses: ["pagada", "pagada", "pagada", "pagada", "pagada", "pendiente", "futura", "futura", "futura", "futura", "futura", "futura"] },
  { name: "FAOV", cadence: "Mensual", deadline: "Regla pendiente de configurar", href: "/declaraciones/faov", statuses: ["pagada", "pagada", "pagada", "pagada", "pagada", "pendiente", "futura", "futura", "futura", "futura", "futura", "futura"] },
  { name: "INCES · Aporte patronal", cadence: "Trimestral", deadline: "Regla pendiente de configurar", href: "/declaraciones/inces", statuses: ["no-aplica", "no-aplica", "pagada", "no-aplica", "no-aplica", "declarada", "no-aplica", "no-aplica", "futura", "no-aplica", "no-aplica", "futura"] },
  { name: "Retención ISLR", cadence: "Mensual", deadline: "Regla configurada por la firma", href: "/declaraciones", statuses: ["pagada", "pagada", "pagada", "pagada", "pagada", "declarada", "futura", "futura", "futura", "futura", "futura", "futura"] },
  { name: "Actividades económicas", cadence: "Mensual · Casa matriz", deadline: "10 días hábiles · período siguiente", href: "/declaraciones/municipales", statuses: ["pagada", "pagada", "pagada", "pagada", "pagada", "pendiente", "futura", "futura", "futura", "futura", "futura", "futura"] },
];
...
export function DeclarationStatusOverview() {
  return (
    <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-10">
      ...
      <section className="mt-6 overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm dark:border-stone-800 dark:bg-stone-900"><div className="flex flex-col gap-3 border-b border-stone-100 p-5 dark:border-stone-800 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold">Control anual de cumplimiento</h2><p className="mt-1 text-sm text-stone-500">Cada marca representa el estado del expediente de ese período.</p></div><div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">{(["pendiente", "declarada", "pagada", "futura"] as Status[]).map((status) => <Legend key={status} status={status} />)}</div></div><div className="overflow-x-auto"><Table className="min-w-[980px] w-full text-left text-sm"><TableHeader className="bg-stone-50 text-xs font-medium text-stone-500 dark:bg-stone-800/70"><TableRow><TableHead className="min-w-72 px-5 py-3">Obligación</TableHead>{months.map((month) => <TableHead className="w-14 px-2 py-3 text-center" key={month}>{month}</TableHead>)}</TableRow></TableHeader><TableBody className="divide-y divide-stone-100 dark:divide-stone-800">{obligations.map((obligation) => <TableRow className="hover:bg-stone-50 dark:hover:bg-stone-800/50" key={obligation.name}><TableCell className="px-5 py-4"><Link className="font-medium hover:text-[#14352d] dark:hover:text-emerald-300" href={obligation.href}>{obligation.name}</Link><p className="mt-0.5 text-xs text-stone-500">{obligation.cadence}</p><p className="mt-1 text-xs text-stone-400">{obligation.deadline}</p></TableCell>{obligation.statuses.map((status, index) => <TableCell className="px-2 py-4 text-center" key={`${obligation.name}-${months[index]}`}><StatusMark status={status} /></TableCell>)}</TableRow>)}</TableBody></Table></div><div className="border-t border-stone-100 bg-stone-50/60 px-5 py-3 text-xs leading-5 text-stone-500 dark:border-stone-800 dark:bg-stone-800/40">Las empresas SPE separarán IVA, Retenciones de IVA e IGTF en dos expedientes por mes y usarán el calendario anual asignado. Los estados mostrados siguen siendo demostrativos.</div></section>
    </div>
  );
}
