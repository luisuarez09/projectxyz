...
export type TeamDirectoryAccount = {
...
type Account = TeamDirectoryAccount;
...
function AccountActions({ account, onEdit, onToggle, onDelete }: { account: Account; onEdit: () => void; onToggle: () => void; onDelete: () => void }) { const { isOpen, ref, setIsOpen } = useDismissableMenu<HTMLDivElement>(); const run = (fn: () => void) => { setIsOpen(false); fn(); }; return <div className="relative inline-block" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()} ref={ref}><button aria-label={`Acciones de ${account.name}`} className="grid size-8 place-items-center rounded-lg hover:bg-stone-100" onClick={() => setIsOpen((value) => !value)} type="button"><MoreHorizontal /></button>{isOpen && <div className="absolute right-0 top-9 z-20 w-56 rounded-xl border bg-white p-1.5 shadow-xl dark:bg-stone-900">{account.kind === "MEMBER" && <button className="menu-action" onClick={() => run(onEdit)} type="button"><Pencil /> Editar datos y accesos</button>}{account.kind === "MEMBER" && <button className="menu-action" onClick={() => run(onToggle)} type="button"><Power /> {account.active ? "Desactivar acceso" : "Reactivar acceso"}</button>}<button className="menu-action text-rose-700" onClick={() => run(onDelete)} type="button"><Trash2 /> {account.kind === "MEMBER" ? "Retirar cuenta" : "Revocar invitación"}</button></div>}</div>; }
